...
Element* Analyzer :: expression(std::ostream& out)
{
    ...
    if(type != Type::NUMBER)    // if it's not a number, then it must be a function name or a bracket
    {
        if(type != Type::NAME_OF_FUNCTION)  // if it's not a function name, then it must be a closing bracket
        {
            ...
            if(idx != -1 || type == Type::OPENING_SQUARE_BRACKET)   // list declaration
        {
            this->next();
            ...
            do
            {
                ...
                if(type == Type::CLOSING_SQUARE_BRACKET &&
                    (idx + 1 >= size || this->tokens[idx + 1]->type == Type::CLOSING_SQUARE_BRACKET
                    || idx + 2 >= size || this->tokens [idx + 2]->type == Type::NAME_OF_FUNCTION))
                {
                    if(idx + 1 < size && this->tokens[idx + 1]->type == Type::CLOSING_SQUARE_BRACKET)  
                    {
                        this->next();
                    }
                    break;
                ...
                if( idx != -1 && type == Type::CLOSING_SQUARE_BRACKET)
                {
                    this->next();
            ...
            return new ListOperation(new Token(Type::NAME_OF_FUNCTION), args, true); // return the list operation and its arguments
        ...
        if(funcPtr && funcPtr->name == LENGTH)
        {
            ...
            if(funcPtr && funcPtr->name == LIST)    // we are looking at a list function declaration
            {
                ...
                listOp = new ListOperation(op, args); // return the list operation and its arguments
            } 
            return new Length(new Token(Type::NAME_OF_FUNCTION),args);
        }
        // -------- LIST --------
        if(funcPtr && funcPtr->name == LIST)    // we are looking at a list function declaration
        {
