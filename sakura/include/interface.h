...
public:
	...
	const std::vector<WordCount> & getWordCounts() const
	{
		return wordCounts;
	}
	std::string wordAt(const int idx) const;
	
	WordsMultiset() {}
	WordsMultiset(const std::vector<WordCount> & sortedWcs) : wordCounts(sortedWcs) {}
...
public:
	...
	int getPercent(std::istream& a, std::istream& b,std::istream& a2) const;
